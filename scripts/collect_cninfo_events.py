#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""collect_cninfo_events.py — A股公告事件历史六关键词采集（cninfo 公告全文检索）

事件族: 权益变动 / 减持计划 / 增持计划 / 高送转 / 股权质押 / 司法冻结
用途: FinAI2.0 事件驱动 alpha 轴补新事件族。标题原文保留，细类归并
      （草案/进展/完成/失效/解除等）由下游主控完成。

数据源:
  cninfo 公告全文检索 POST https://www.cninfo.com.cn/new/hisAnnouncement/query
  （单路全市场: column 参数不做市场过滤，含北交所/退市股，不剔）

实测口径 (与 collect_esop.py / collect_letters.py 同构):
...
输出 (out dir, 默认 ./cninfo_events):
  {kw}_{YYYYMM}.parquet    按月分片事件表:
                           ann_date(YYYY-MM-DD), ts_code(裸码6位), kw(六关键词之一),
...
用法:
  python3 collect_cninfo_events.py --out ./cninfo_events            # 2015-01 ~ 2024-12 全量（可续跑）
  python3 collect_cninfo_events.py --out ./cninfo_events --kw 减持计划 --start 2020-01-01 --end 2020-12-31
...
KEYWORDS = ["权益变动", "减持计划", "增持计划", "高送转", "股权质押", "司法冻结"]
...
class Collector:
    ...
    def collect_month(self, kw, d1, d2):
        """整月采集入口：产出 cninfo_events/{kw}_{YYYYMM}.parquet。"""
...
def main():
    ...
    ap.add_argument("--out", default="./cninfo_events")
