...
### 2.2 迅投底层委托状态码（order_status）与常见柜台错误

依据迅投官方数据字典及 `xtconstant` 规范：

| `order_status` 数值 | `xtconstant` 常量名 | 状态中文含义 | 实战撮合与风控处理逻辑 |
|---|---|---|---|
| **`48`** | `ORDER_UNREPORTED` | 未报（本地已生成但尚未送达柜台） | 检查本地网络与 MiniQMT 进程状态 |
| **`49`** | `ORDER_WAIT_REPORTING` | 待报（柜台排队等待发往交易所） | 正常瞬时过渡态，无需干预 |
| **`50`** | `ORDER_REPORTED` | 已报（交易所主机已接收在队列中） | 可撤单状态；挂接成交与撤单回报监听 |
| **`51`** | `ORDER_REPORTED_CANCEL` | 已报待撤（撤单指令已发出等待确认） | 锁定本地订单状态，禁止重复发撤单指令 |
| **`52`** | `ORDER_PARTSUCC_CANCEL` | 部成待撤（部分成交且剩余在撤单中） | 准备根据剩余成交量更新账本持仓 |
| **`53`** | `ORDER_PART_CANCEL` | 部撤（部分成交，剩余部分已成功撤单） | 终止订单生命周期，以已成交数量结算 |
| **`54`** | `ORDER_CANCELED` | 已撤（全额撤单成功，无成交） | 释放全部冻结资金，订单完结 |
| **`55`** | `ORDER_PART_SUCC` | 部成（已发生部分撮合成交，剩余仍排队） | 账本先行推导已成交头寸 |
| **`56`** | `ORDER_SUCCEEDED` | 已成（全部申报数量撮合成交完毕） | 订单成功完结，账本记录完整流水 |
| **`57`** | `ORDER_JUNK` | 废单（柜台或交易所风控直接驳回） | 提取 `order.status_msg` 原因（超价格笼子/资金不足/停牌等），标记失败 |
| **`255`** | `ORDER_UNKNOWN` | 未知状态（超时未决） | **绝对禁止盲目补单**！标记为 `IN_DOUBT`，进入对账轮询 |

> **勘误说明**：前述草稿中将 48/49/50 笼统描述为“错误码”，经查证迅投数据字典，**48~57 实为委托生命周期的标准状态码（order_status）**，其中 **57 为废单（Junk）**，真正的柜台拒单原因需从 `order.status_msg` 或异步报错 `on_order_error` 中解析。已在上述表格中按官方数据字典规范进行了条文级订正。
